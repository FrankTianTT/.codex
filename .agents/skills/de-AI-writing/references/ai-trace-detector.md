...
核心问题不是破折号本身，而是**同一功能反复出现**——全文破折号全在扮演「概念——解释」或「结论——原因」，节奏单调。

下面按破折号承担的**语义功能**分类，每类给出改写方向。关键原则：**不要用冒号替代破折号**（冒号有自己的规则，见 SKILL.md §2），优先用逗号、句号、或拆句。

#### A. 「概念——解释/展开」

破折号前抛出一个判断，破折号后展开说明。最常见也最容易被滥用。

| Before | After | 手法 |
|--------|-------|------|
| `GAN 曾长期主宰图像生成任务——让两个网络互相对抗，一个生成图片、一个鉴别真伪` | `GAN 曾长期主宰图像生成任务，它让两个网络互相对抗，一个生成图片、一个鉴别真伪` | 逗号 + 补主语（`它`），把解释性内容接成自然后续 |
| `生成器还容易偷懒——学会生成少数几种花样就停滞不前` | `生成器还容易偷懒，学会生成少数几种花样就停滞不前` | 逗号直连，无需停顿 |
| `极限形态——具体来说，是把 VAE 的「压缩」步骤固定为……` | `极限形态。把 VAE 的「压缩」步骤固定为……` | 句号拆成两句；删掉「具体来说」（破折号本身就是「展开说」的信号，再加「具体来说」是双重标记） |
| `唯一的区别是 encoder 被固定——它不参与训练，只按预设的噪声计划执行` | `唯一的区别是 encoder 被固定：不参与训练，只按预设的噪声计划执行` | 极少数情况用冒号确实更顺（引出分项说明而非单句展开），但要确认不触发 SKILL.md §2 的「概念：解释」模板感 |

#### B. 「结论——原因」

破折号前下结论，破折号后给原因。AI 特别喜欢这种「先砸结论再补理由」的节奏。

| Before | After | 手法 |
|--------|-------|------|
| `压缩器就被推向不提供任何信息的方向——因为提供信息反而会受惩罚` | `压缩器就被推向沉默。因为提供信息反而受惩罚，最优策略就是不提供。` | 句号断开。让原因独立成句，补一句点明机制（「最优策略就是不提供」），读者不必自己填推理跳步 |
| `这个模型很难训练——损失函数的地貌太复杂了` | `这个模型很难训练，损失函数的地貌实在太复杂了` | 逗号直连。原因本身够短时不需要破折号的「停顿宣告」效果 |

#### C. 「抽象概念——具象画面」

破折号前给一个抽象判断，破折号后用画面/比喻托住。

| Before | After | 手法 |
|--------|-------|------|
| `生成的样本偏「模糊」——人脸像隔了层毛玻璃` | `生成的样本偏「模糊」，人脸像隔了层毛玻璃` | 逗号。抽象判断和具象画面之间不需要额外的停顿宣告 |
| `反向过程本质上是随机微分方程的离散化——每一步添加适量噪声修正累积的逼近误差` | `反向过程本质上是随机微分方程的离散化：每一步添加适量噪声，修正累积的逼近误差` | 偶尔用冒号可行（后文是机制展开而非单一画面），但更常见的是拆成两句或用逗号 |

#### D. 「叙述中插入补充/限定」

破折号在叙述流中插入一个补充信息或限定条件。

| Before | After | 手法 |
|--------|-------|------|
| `DDPM 实践中取 T=1000——关键在于 ᾱ_T ≈ 0，保证前向终点是纯噪声——T 本身不是硬约束` | `DDPM 实践中取 T=1000。关键在于 ᾱ_T ≈ 0（保证前向终点是纯噪声），T 本身不是硬约束。` | 句号断开主线；补充信息用括号（更轻量，不抢戏）；删掉一个破折号减少嵌套 |

#### E. 应该保留的破折号

破折号本身不是坏东西。以下场景保留破折号效果更好：

- **收束节奏**：`再将压缩层数推向无穷——得到的就是扩散模型。`（破折号前是铺垫、后是落槌，换成句号或逗号会丢掉「蓄力→释放」的节奏）
- **语意转折**：`ELBO 的连续形式是一个与噪声调度无关的 Riemann 积分——这证明了扩散模型不是 HVAE 的近似，而是它的精确极限。`（后文是对前文的「拔高」，破折号制造了揭示感，换成句号会变平）
- **真正需要双破折号插入的补充**：`两类关键失败模式——模式坍塌和后验坍塌——不需要前置知识。`（括号也可以，但破折号在此处的停顿感更自然）

**判断标准**：删掉破折号、换成逗号/句号/括号后，如果语义和节奏都没受损，说明原破折号是装饰性的，应该删。只有当换掉后会丢失某种语感（收束、转折、强调），才保留。
...
不要用 Markdown 引用块（`>`）套加粗标签（`**图释**`、`**参数化注记**`）来写图片说明。这是 PPT 讲义的排版习惯，不是文章正文的写法。

- **信号**：图片下方或上方出现 `> **图释**：…` 或 `> **参数化注记**：…` 格式的引用块。
- **修法**：去掉 `>` 和加粗标签，改成自然段落。直接说「这张图展示了……」或把图片说明融进上下文。

| Before | After |
|--------|-------|
| `> **图释**：三列架构从左到右展示了从 HVAE 到扩散模型的递进关系。左列是完整 HVAE……` | `这张图从左到右展示了从 HVAE 到扩散模型的递进关系。左列是完整 HVAE……` |
| `> **参数化注记**：这里 x̂_θ 使用的是数据预测参数化……` | `这里 x̂_θ 使用的是数据预测参数化……` |

### 16) 内联标题垂直列表（”小标题：解释”一条条）
