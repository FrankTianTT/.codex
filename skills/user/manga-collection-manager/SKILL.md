...
description: |
  维护个人 NAS 漫画/同人志馆藏，覆盖来源获取、解压、语言分类、去重、画师文件夹重组、压缩与迁移。当用户要求整理或管理漫画馆藏，处理魂+/南+/北+/ibm5100、ImoutoHeaven/AList、E-hentai 来源，或对同人志执行分类、去重和重组时使用，可隐式触发。隐式触发后仅做只读盘点、分类分析和 dry-run 预览；下载、删除、覆盖、迁移、压缩等写操作必须先展示准确计划并取得用户明确授权。
---
