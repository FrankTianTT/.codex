---
name: image-reader
description: 调用智谱或 MiniMax 第三方视觉模型做结构化图片提取。仅在用户显式调用本 Skill、明确要求第三方模型对照，或现有项目流程明确依赖这些脚本时使用。普通看图使用 Codex 原生视觉；本地 OCR 使用 local-ocr；项目级渲染验证使用项目的 vision-tools。
---
...
- 本 Skill 会把完整图片发送到智谱；失败时可能发送到 MiniMax。
- 执行前说明目标服务并取得用户同意。身份证件、合同、聊天记录、未公开数据等敏感图片默认不外发。
- 普通图片理解优先使用 Codex 原生视觉；纯文字 OCR 优先使用 `local-ocr`。
- 不因“看图、OCR、分析截图”等普通请求自动调用本 Skill。
